  ...
  return (
    <Container className="mt-10 lg:mt-20 bg-tech_white p-5 lg:p-7 rounded-md">
      <div className="flex items-center gap-5 justify-between mb-10">
        ...
        <Link
          href={"/shop"}
          className="text-sm font-semibold tracking-wide hover:text-tech_blue hoverEffect"
        >
          View all
        </Link>
      </div>
      <div className="flex items-center gap-2.5 justify-between">
        {brands?.map((brand) => (
          <Link
            ...
            className="bg-white border hover:border-tech_blue w-36 h-24 flex items-center justify-center rounded-md overflow-hidden hoverEffect"
          >
            {brand?.image && (
              <Image
