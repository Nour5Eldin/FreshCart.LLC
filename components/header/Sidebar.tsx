  ...
  return (
    ...
    >
      <motion.div
        ...
        ref={sidebarRef}
        className="min-w-72 max-w-96 bg-tech_blue z-50 h-screen p-10 border-r border-r-tech_blue flex flex-col gap-6"
      >
        <div className="flex items-center justify-between">
          <Logo />
          <button
            onClick={onClose}
            className="hover:text-tech_yellow hoverEffect"
          >
            <X />
          </button>
        </div>
          ...
          {headerData?.map((item) => (
            <Link
              ...
              className={`hover:text-tech_orange hoverEffect ${
                pathname === item?.href && "text-tech_yellow"
              }`}
            >
              {item?.title}
            </Link>
